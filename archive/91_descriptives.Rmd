...
```{r}
mycol_lda <- text_lda_beta %>% gg_color_select(cat = topic_name, pal = pal_tp)
```
...
text_lda_gamma %>%
  ...
  plot_summary_timeline(y1 = weight, y2 = weight_rel, t = PY, t_min = PY_min, t_max = PY_max, by = topic_name,  label = TRUE, pal = pal_tp, 
...
for(i in 1:nrow(com_names_cit)){
  kb_sum <- kb_sum %>%
    pack_rows(paste0('Community ', i, ': ', com_names_cit[i, 'com_name']), (i*10-9),  (i*10), label_row_css = "background-color: #666; color: #fff;") 
  }
...
kb_sum %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), font_size = 10)
```
